...
class salary extends Main {
	...
	function summary() {
		...
		if (isset($_GET["date_from"]) && $_GET["date_from"] != "") {
			$date_from = "01-" . $_GET["date_from"];
			$date_from = date("Y-m-d", strtotime($date_from));
			$day_fee = "01-" . $_GET["date_from"];
			$day_fee = date("Y-m-d", strtotime($day_fee));
		...
		$array_group = array(""=>array("id"=>"","name"=>"Chọn xưởng"));
		$array_group += $this->Group->find("all", array("fields" => "id,name"));
		...
		*/
		...
		//lấy điều kiện công tác phí
		$dieukien_fee = "";
		$dk_chamcong = "";
		$day_fee_to = "";
		if($day_fee != "") 
		{
			$day_fee_to = date("t-m-Y",strtotime($day_fee));
			$dieukien_fee = " WHERE thang='$day_fee'";
			$dk_chamcong = "WHERE day >= '$day_fee' AND day <= '$day_fee_to'";
		}
		//BEGIN: SQL TRUY VẤN BẢNG CSDL
		$sql_col_user = "id, user_code, fullname, id_number, bank_account, shift_type, date_join";
		$sql_user = "SELECT $sql_col_user FROM `scm_users` $dk";
		
		$sql_col_salary = "id_user,thang, luong_coban, trachnhiem, kiemnhiem, phucap_luong,luong_baohiem, chuyencan,phucap_ca3, phucap_dienthoai";
		$sql_salary = "SELECT $sql_col_salary FROM `scm_salary` $dieukien_fee";
		
		$sql_user_salary = "SELECT A.*, B.* FROM ($sql_user) AS A LEFT JOIN ($sql_salary) AS B ON A.id = B.id_user";
		
		$sql_col_fee = "id_user as id_user_fee, landi,solit, dongia,thanhtien";
		$sql_fee = "SELECT $sql_col_fee FROM `scm_fee` $dieukien_fee";
		
		$sql_user_salary_fee = "SELECT C.*, D.* FROM ($sql_user_salary) AS C LEFT JOIN ($sql_fee) AS D ON C.id = D.id_user_fee";
		//END: SQL TRUY VẤN BẢNG CSDL
		...
		";
		$sql_col_chamcong = "id_user AS id_user_cc, user_fullname $sql_col_chamcong_sum";
		$sql_chamcong = "SELECT $sql_col_chamcong FROM scm_chamcong $dk_chamcong GROUP BY id_user, user_fullname, user_code ORDER BY id_user ASC";
		...
		$sql_tonghop_chamcong_luong = "SELECT A.*, $sql_col_tonghop FROM ($sql_user_salary_fee) AS A LEFT JOIN ($sql_chamcong) AS B ON A.id = B.id_user_cc ";
		//echo $sql_tonghop_chamcong_luong;
		
		//echo $sql_user_salary_fee;
		
		$array_salary = $this->User2->query($sql_tonghop_chamcong_luong);
