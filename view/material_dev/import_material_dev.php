...
//lấy mảng array_material_import từ view truyền qua
$id_import = "";
$id_warehouse = "";
$id_customer = "";
$date = "";
...
$id_user = "";
if ($array_material_import) {
	$id_import = $array_material_import[0]["id"];
	$id_warehouse = $array_material_import[0]["id_warehouse"];
	$id_customer = $array_material_import[0]["id_customer"];
	$date = $array_material_import[0]["day"];
	$code = $array_material_import[0]["code"];
	$delivery_man = $array_material_import[0]["delivery_man"];
	$delivery_vans_number = $array_material_import[0]["delivery_vans_number"];
	$id_user = $array_material_import[0]["id_user"];
}
...
//load selectbox kho
$str_selectbox_kho_material = $this->Template->load_selectbox_basic(array("name" => "MaterialImport[id_warehouse]", "style" => "width:300px"), $array_warehouse, $id_warehouse);
$str_form_row_material .= $this->Template->load_form_row(array("title" => "Nhập kho", "input" => $str_selectbox_kho_material));

//load input date
$str_input_day_material = $this->Template->load_textbox(array("name" => "MaterialImport[day]", "value" => $date, "id" => "input_date", "style" => "width:200px"));
$str_form_row_material .= $this->Template->load_form_row(array("title" => "Ngày", "input" => $str_input_day_material));

//tao dong chua id an
$str_input_id_material = $this->Template->load_hidden(array("name" => "MaterialImport[id]", "value" => $id_import));

//load input mã phiếu
$str_input_code_material = $this->Template->load_textbox(array("name" => "MaterialImport[code]", "value" => $code, "style" => "width:200px"));
...
$str_selectbox_supplier_material = $this->Template->load_selectbox_basic(array("name" => "MaterialImport[id_customer]", "style" => "width:300px"), $array_customer, $id_customer);
...
$str_input_delivery_man = $this->Template->load_textbox(array("name" => "MaterialImport[delivery_man]", "value" => $delivery_man, "style" => "width:200px"));
$str_form_row_material .= $this->Template->load_form_row(array("title" => "Người giao hàng", "input" => $str_input_delivery_man));

//load input số xe
$str_input_delivery_vans_number = $this->Template->load_textbox(array("name" => "MaterialImport[delivery_vans_number]", "value" => $delivery_vans_number, "style" => "width:200px"));
...
$str_selectbox_user_material = $this->Template->load_selectbox_basic(array("name" => "MaterialImport[id_user]", "style" => "width:300px"), $array_user, $id_user);
...
$str_import_row = "";
$stt = 0;
if ($array_material_import_detail) {
	foreach ($array_material_import_detail as $material_import_detail) {

		//BEGIN: Lấy thông tin material_import_detail
		$stt++;
		$id_material_import_detail = $material_import_detail["id"];
		$id_material = $material_import_detail["id_material"];
		$id_material_import = $material_import_detail["id_material_import"];
		$material_code = $material_import_detail["code"];
		$material_name = $material_import_detail["material_name"];
		$material_num = $material_import_detail["num"];
		$material_price = $material_import_detail["price"];
		$type = $material_import_detail["type"];
		$status = $material_import_detail["status"];
		//END: lấy thông tin material_import_detail
		...
		$str_hidden_id_material_import = $this->Template->load_hidden(array("name" => "data[$index][id_material_import]", "value" => $id_material_import, "style" => "width:100px"));
		...
		//tạo link xóa
		$str_link_delete = $this->Template->load_link("del", "Xóa", "/material/del_import/$id_material_import/$id_material_import_detail");

		$str_hidden = $str_hidden_material_code . $str_hidden_material_code . $str_hidden_material_status;
		$str_hidden .= $str_hidden_id_material . $str_hidden_id_material_import;
		...
		$str_import_row .= $this->Template->load_table_row($array_table_material_row);

		$index++;

	} //END: foreach ($array_material_import_detail as $material_import_detail)
} //END: if ($array_material_import_detail
//END: lấy các dòng đã có

// echo "Header:" . $str_import_row;
...
if ($array_material) {
	...
	foreach ($array_material as $material) {
		...
		// $str_hidden_id_material_import = $this->Template->load_hidden(array("name" => "data[$index][id]", "value" => $material["id_import_detail"], "style" => "width:100px"));
...
$str_material = $this->Template->load_table($str_material_header . $str_import_row . $str_material_row . $str_last_row);
//load form
