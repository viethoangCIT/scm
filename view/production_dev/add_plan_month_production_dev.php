...
<?php
	...
	$title_header = " Nhập Kế Hoạch Sản Xuất Theo Tháng";
	...
	// LOAD FORM
	$str_form_production = $this->Template->load_form(array("method" => "GET", "id" => "form_seach", "action" => "/production/add_plan_month"),$str_input_row);
	...
	//Dùng hàm load_selectbox của đối tượng Template để selectbox
	$str_selectbox_code_customer = $this->Template->load_selectbox(array("name" => "production[id_customer]" ,"style" => "width:300px"), $array_customer);
	
	//$str_hidden_id_product = $this->Template->load_hidden(array("name" => "production[id_product]", "id"=>"id_product" ,"style" => "width:100px"));

	$str_hidden_id_factory = $this->Template->load_hidden(array("name" => "production[id_factory]", "id"=>"id_factory" ,"style" => "width:100px"));
	
	$str_hidden_id_manufactory = $this->Template->load_hidden(array("name" => "production[id_manufactory]", "id"=>"id_manufactory" ,"style" => "width:100px"));
	
	$str_hidden_id_cat = $this->Template->load_hidden(array("name" => "production[id_cat]", "id"=>"id_cat" ,"style" => "width:100px"));
	
	$str_hidden_month = $this->Template->load_hidden(array("name" => "production[month]", "id"=>"month" ,"style" => "width:100px"));
	
	$str_hidden_songay = $this->Template->load_hidden(array("name"=>"production[songay]", "id"=>"songay", "value"=>$songay));
	
	$str_hidden = "$str_hidden_id_factory . $str_hidden_id_manufactory . $str_hidden_id_cat . $str_hidden_month . $str_hidden_songay";
	...
	if($array_delivery)
	{
		foreach($array_delivery as $delivery)
		{
			...
			$link_xoa = "/production/del_plan_month/$id_delivery";
	...
	// LOAD FORM
	$str_form_production = $this->Template->load_form(array("method" => "POST", "id" => "form_nhap", "action" => "/production/add_plan_month"),$str_table_production);
