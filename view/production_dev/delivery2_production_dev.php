...
<?php
	$title_header = " Xem Kế Hoạch Giao Hàng";
	...
	if($array_delivery)
	{
		foreach($array_delivery as $delivery)
		{
			...
			$link_chitiet = "/production/add_delivery?id_factory=$id_factory&id_manufactory=$id_manufactory&id_cat=$id_cat&month=$month&id_delivery=$id_delivery";
			...
			$array_row_production["col7"] = array($delivery["product_name"], array("style"=>"width:100px"));
		...
		}//End: foreach($array_delivery as $delivery)
			
			$array_row_production = NULL;
			$array_row_production["col1"] = array("", array("align"=>"center","colspan"=>"5"));
			$array_row_production["col2"] = array("Tổng", array("align"=>"center"));
			$array_row_production["col3"] = array(number_format($tong), array("align"=>"center","colspan"=>"2"));
			$str_row_production .= $this->Template->load_table_row($array_row_production);
		
	}//End: if($array_delivery)
	...
	// LOAD FORM
	$str_form_production = $this->Template->load_form(array("method" => "GET", "id" => "form_seach", "action" => "/production/delivery"),$str_input_row . $str_table_production);
